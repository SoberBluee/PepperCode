...
class Pepper():
    ...
    def setup(self):
        ...
        try:
            ...
            self.explore = ALProxy("ALNavigationProxy", self._robotIP, self._PORT)
...
#        self.listen(data.data)
    ...
    def explore_callback(self,data):
        rospy.loginfo("%s", data.data)

        if data.state:
            self.explore.explore(,1)
            print("Explore Started !")
            self.explore.startLocalization()
            print("Localization Started!")
            self.explore.saveExploration()
        elif data.state == False:
            self.stopExploration()
            self.stopLocalization()

            print("Explore and Localization has stopped")
        else:
            print("Wrong state")
    ...
    def update(self):
        ...
        rospy.Subscrier("/cmd_vel_mux/input/teleop", Twist, self.twist_callback)
        rospy.Subscriber("pepper/exploreNav",explore_msgs.msg.exploreNav.msg, self.explore_callback)
